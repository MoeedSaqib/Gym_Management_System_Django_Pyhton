...
def gallery(request):
	gallery=models.Gallery.objects.all().order_by('id')#order by descending id
...
def pricing(request):
	...
	return render(request, 'pricing.html',{'pricing':pricing , 'dfeatures':dfeatures})
...
stripe.api_key ='sk_test_51NSm3KSCXJpD51aG9Eb3bFFluQ90wG2HhCPa51CCQdmz9M2qoLN4RpDOUsUz9oxriJSdjaBrftpjnEzbTxMW2FRy00VA0VihOt'
...
def pay_success(request):
    ...
    models.Subscription.objects.create(
	    plan=plan, user=user , price=plan.price
	)
    subject='Order Email'
    html_content=get_template('orderemail.html').render({'title':plan.title})
    from_email='bluecandy852@gmail.com'
